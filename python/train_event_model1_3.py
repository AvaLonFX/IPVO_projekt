# python/train_event_model_v1_3_group_split.py
# V1.3: Group split by daily_video_id (no leakage across same daily video),
# head-only training + ImageNet normalization + class-weighted loss.
...
MODEL_OUT = Path("python/models/event_resnet18_v1_3_group_split.pt")
...
def group_split(samples: List[Sample], seed: int = 42) -> Tuple[List[Sample], List[Sample], List[Sample]]:
    """
    Split by group (daily_video_id) to avoid leakage.
    Rough target: ~70% train, ~15% val, ~15% test by number of samples.
    """
    ...
    # If train ended up too small (few groups), fall back: move some from val to train
    if len(train) < 10 and len(val) > 10:
        train.extend(val[:10])
        val = val[10:]
...
class ClipFramesDataset(Dataset):
    ...
    def _pick_frames_even(self, frames_dir: Path) -> List[Path]:
    ...
    def __getitem__(self, idx):
        s = self.samples[idx]
        frame_paths = self._pick_frames_even(s.frames_dir)
...
def main():
    ...
    # Class-weighted loss from TRAIN distribution
    dunk_n, three_n = count_labels(train_s)
    total = dunk_n + three_n
    # weights inversely proportional to frequency
    w_dunk = total / max(dunk_n, 1)
    w_three = total / max(three_n, 1)
    class_weights = torch.tensor([w_dunk, w_three], dtype=torch.float32).to(device)
    print("Class weights [dunk, three]:", class_weights.tolist())

    criterion = nn.CrossEntropyLoss(weight=class_weights)
    opt = torch.optim.Adam(model.head.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    ...
    torch.save({
        ...
        "arch": "resnet18_temporal_avg_head_only_group_split",
